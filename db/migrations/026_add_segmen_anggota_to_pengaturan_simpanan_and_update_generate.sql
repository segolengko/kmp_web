...
ADD COLUMN IF NOT EXISTS segmen_anggota VARCHAR(30);
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'ck_pengaturan_simpanan_segmen_anggota'
    ) THEN
        ALTER TABLE pengaturan_simpanan
        ADD CONSTRAINT ck_pengaturan_simpanan_segmen_anggota
            CHECK (
                segmen_anggota IS NULL
                OR segmen_anggota IN (
                    'BIASA_AKTIF',
                    'LUAR_BIASA_AKTIF',
                    'LUAR_BIASA_PASIF'
                )
...
CREATE INDEX IF NOT EXISTS idx_pengaturan_simpanan_segmen_anggota
    ON pengaturan_simpanan (jenis_simpanan_id, segmen_anggota, aktif, berlaku_mulai, berlaku_sampai);

UPDATE pengaturan_simpanan
SET segmen_anggota = 'BIASA_AKTIF'
WHERE segmen_anggota IS NULL
  AND jenis_anggota = 'BIASA';
...
BEGIN
    ...
    VALUES (
        ...
        0,
        'PROSES',
        'Generate simpanan wajib bulanan dengan nominal per periode dan segmen anggota.',
        p_dibuat_oleh
    )
    ...
    WITH kandidat AS (
        SELECT
            ...
            CASE
                ...
                WHEN ps_segmen.id IS NOT NULL THEN FORMAT('PENGATURAN_SEGMEN_%s', COALESCE(
                    CASE
                        WHEN a.jenis_anggota = 'BIASA' AND a.status_anggota = 'AKTIF' THEN 'BIASA_AKTIF'
                        WHEN a.jenis_anggota = 'LUAR_BIASA' AND a.status_anggota = 'AKTIF' THEN 'LUAR_BIASA_AKTIF'
                        WHEN a.jenis_anggota = 'LUAR_BIASA' AND a.status_anggota = 'PASIF' THEN 'LUAR_BIASA_PASIF'
    ...
    SET
        ...
        catatan = FORMAT(
            'Generate simpanan wajib selesai. Auto lunas anggota biasa aktif: %s. Tagihan manual segmen lain: %s. Anggota tanpa nominal: %s.',
            v_total_tagihan_auto_lunas,
            v_total_tagihan_manual,
